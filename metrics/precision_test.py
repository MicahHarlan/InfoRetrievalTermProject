...
tests = pd.read_csv(
    '/metrics/pg/tag_prompt_titles.csv')
...
for index, row in tqdm(tests.iterrows()):
    ...
    if check_format(result_text):
        is_formatted = 1
    llm_format_results = pd.concat(
        [llm_format_results, pd.DataFrame({'Prompt': [row['prompt']], 'isResponseFormatted': [is_formatted]})],
        ignore_index=True)
    clear_chat()

llm_results.to_csv('llm_results_1.csv')
llm_format_results.to_csv('llm_format_results_1.csv')
